        ...
        FACTORY_EXECUTE_REPLY_ID => {
            ...
            match res.data {
                Some(data) => {
                    ...
                    // Unlike the built-in new-token path, a custom factory can
                    // intentionally start with no minted voting tokens and make
                    // them available later (for example, through a bonding
                    // curve). In that case the DAO starts inactive until users
                    // acquire and stake enough tokens to meet the configured
                    // active threshold.
