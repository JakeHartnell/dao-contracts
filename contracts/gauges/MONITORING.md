...
| `GaugeBudgetUnderfunded` | critical | Before execution, sum adapter-produced native/CW20 obligations by asset and compare with the DAO-controlled source balance; fire when any available balance is below the required amount | Verified balance covers the next obligation with the governance-approved reserve margin |
...
The canary dashboard must show epoch deadline and execution outcome, selected
and message counts, all health fields, voting-power hook events, reset cursor
